...
const displayTitles = (arr) => {
    let newArr = [];
    return newArr = arr.map(x => x = { title: x.title });
};

const isRentedBook = (arr) => {
    let res = true;
    arr.filter(x => {
        if (x.rented == 0) return res = false;
    });
    return res;
};
...
const displayTheLeastRentedBook = (arr) => {
    let mostRented = 9999999999999999;
    arr.filter(x => {
        if (x.rented < mostRented) mostRented = x.rented;
    });
    return mostRented;
...
const deleteBookById = (arr) => {
    let newArr = [];
    return newArr = arr.filter(x => x.id != 133712);
};

console.log('titres des livres :\n', displayTitles(books), '\n----------------------');
console.log('tous les livres empruntés au moins une fois :\n', isRentedBook(books), '\n----------------------');
console.log('livre le plus emprunté :\n', displayTheMostRentedBook(books), '\n----------------------');
console.log('livre le moins emprunté :\n', displayTheLeastRentedBook(books), '\n----------------------');
